...
public class IndicadorCalculadoService {
    ...
    public IndicadorCalculado getIndicadorCalculado(Empresa empresa, Indicador indicador, String periodo) {
        ...
        try {
            trx.begin();
            List<IndicadorCalculado> indicadoresCalculados = this.em
                    .createQuery("Select i FROM IndicadorCalculado i WHERE i.indicador_id = :indicador_id and c.empresa_id = :empresa_id and c.periodo = :periodo", IndicadorCalculado.class)
                    .setParameter("indicador_id", indicador.getId()).setParameter("empresa_id", empresa.getId()).setParameter("periodo", periodo).getResultList();
